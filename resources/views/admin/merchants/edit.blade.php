    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted small">Update merchant information</p>
            </div>
            <div>
                <a href="{{ route('admin.merchants.show', $merchant->merchant_id) }}" class="btn btn-info text-white me-2">
        ...
        <div class="card">
            <div class="card-header bg-white">
                <i class="fas fa-edit"></i> Edit Merchant: {{ $merchant->business_name }}
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.merchants.update', $merchant->merchant_id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                            ...
                            <select name="owner_id" id="owner_id"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ old('owner_id', $merchant->owner_id) == $user->id ? 'selected' : '' }}>
                                        {{ $user->full_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="branch_name" id="branch_name"
                                ...
                                value="{{ old('branch_name', $merchant->branch_name) }}">
                            @error('branch_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select name="category_id" id="category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->category_id }}"
                                        {{ old('category_id', $merchant->category_id) == $category->category_id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="province_id" id="province_id"
                                ...
                                @foreach ($provinces as $province)
                                    <option value="{{ $province->province_id }}"
                                        {{ old('province_id', $merchant->province_id) == $province->province_id ? 'selected' : '' }}>
                                        {{ $province->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="street_address" id="street_address"
                                ...
                                value="{{ old('street_address', $merchant->street_address) }}" required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="city" id="city"
                                ...
                                value="{{ old('city', $merchant->city) }}" required>
                            @error('city')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" id="status"
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status }}"
                                        {{ old('status', $merchant->status) == $status ? 'selected' : '' }}>
                                        {{ ucfirst($status) }}
                                    </option>
                                @endforeach
                            </select>
